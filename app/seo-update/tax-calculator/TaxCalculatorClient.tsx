...
export default function TaxCalculatorClient() {
  ...
  const result = useMemo(() => {
    ...
    const rebateThreshold = regime === "new" ? NEW_REGIME_REBATE_THRESHOLD : OLD_REGIME_REBATE_THRESHOLD;
    ...
    return { standardDeduction, taxableIncome, taxBeforeRebate, rebate, taxAfterRebate, cess, totalTax, takeHome, effectiveRate };
    ...
    <div className="max-w-content mx-auto px-6 py-14">
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-8">
        <div className="rounded-2xl border border-line dark:border-white/10 bg-surface dark:bg-white/[0.03] p-6">
          <div className="flex gap-2 mb-6">
            <button
              ...
              className={`flex-1 rounded-xl py-3 text-sm font-medium transition-colors ${
                regime === "new" ? "bg-indigo text-white" : "border border-line dark:border-white/10 text-ink dark:text-white hover:border-indigo"
              }`}
            >
              New Regime
            </button>
            <button
              ...
              className={`flex-1 rounded-xl py-3 text-sm font-medium transition-colors ${
                regime === "old" ? "bg-indigo text-white" : "border border-line dark:border-white/10 text-ink dark:text-white hover:border-indigo"
              }`}
            >
              Old Regime
            </button>
          ...
          {regime === "old" && (
            <>
              <label htmlFor="other-deductions" className="block text-sm text-ink/80 dark:text-white/80 mb-2">
          ...
          <div className="rounded-xl bg-paper dark:bg-[#101118] border border-line dark:border-white/10 p-5 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono tabular text-ink dark:text-white">{formatINR(result.standardDeduction)}</span>
            </div>
            {regime === "old" && deductions > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-muted">Other deductions</span>
                <span className="font-mono tabular text-ink dark:text-white">{formatINR(deductions)}</span>
              </div>
            )}
            <div className="flex justify-between text-sm pt-2 border-t border-line dark:border-white/10">
              ...
              <span className="font-mono tabular text-ink dark:text-white">{formatINR(result.taxableIncome)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted">Tax before rebate</span>
              <span className="font-mono tabular text-ink dark:text-white">{formatINR(result.taxBeforeRebate)}</span>
            </div>
            {result.rebate > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-muted">Rebate (Sec 87A)</span>
                <span className="font-mono tabular text-indigo">− {formatINR(result.rebate)}</span>
              </div>
            )}
            <div className="flex justify-between text-sm">
              <span className="text-muted">Health &amp; Education Cess (4%)</span>
              <span className="font-mono tabular text-ink dark:text-white">{formatINR(result.cess)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl bg-ink dark:bg-white/[0.06] p-6">
            <p className="text-xs font-mono uppercase tracking-widest text-paper/60 dark:text-white/60 mb-2">Total tax payable</p>
            <p className="font-mono text-3xl tabular text-paper dark:text-white mb-1">{formatINR(result.totalTax)}</p>
            <p className="text-sm text-paper/60 dark:text-white/60">{result.effectiveRate.toFixed(1)}% effective rate</p>
          </div>

          <div className="rounded-2xl border border-line dark:border-white/10 bg-surface dark:bg-white/[0.03] p-6">
            <p className="text-xs font-mono uppercase tracking-widest text-muted mb-2">Take-home (annual)</p>
            <p className="font-mono text-2xl tabular text-ink dark:text-white">{formatINR(Math.max(0, result.takeHome))}</p>
            <p className="text-sm text-muted mt-1">≈ {formatINR(Math.max(0, result.takeHome) / 12)} / month</p>
          </div>
